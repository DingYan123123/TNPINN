...
class CNNLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, out_size, conv_channels=conv_channels, kernel_size=kernel_size, pool_size=pool_size, dropout_rate=0.3):
...
def calculate_uncertainty_metrics(y_true, ci_lower, ci_upper, alpha=0.05):
    ...
    # 计算PINMW (Prediction Interval Normalized Mean Width)
    interval_width = ci_upper - ci_lower
    y_range = np.max(y_true) - np.min(y_true)
    pinmw = np.mean(interval_width) / y_range if y_range != 0 else 0

    # 计算CWC (Coverage Width-based Criterion)
...
def plot_probability_density(predictions_array, y_true, sample_idx=0, step_idx=0, battery_name="Battery"):
    ...
    Args:
        predictions_array: 蒙特卡洛预测结果 [mc_samples, N, forecast_steps]
        y_true: 真实值 [N, forecast_steps]
        sample_idx: 样本索引
        step_idx: 预测步长索引
        battery_name: 电池名称
...
def plot_multiple_probability_densities(all_predictions, all_y_true, battery_names, sample_idx=0, step_idx=0):
    """
    绘制多个电池的概率密度图，类似论文中的Figure 6

    Args:
        all_predictions: 所有电池的预测结果列表
        all_y_true: 所有电池的真实值列表
    ...
    fig.suptitle('Probability density of the predicted points at the 100th cycle (CNN-LSTM)',
...
def mc_uncertainty_estimation(model, input_data, mc_samples=100):
    ...
    return mean_pred, std_pred, ci_lower, ci_upper, predictions  # 返回predictions用于密度图
...
# 交叉验证
maes, rmses = [], []
all_uncertainty_metrics = []  # 存储所有电池的不确定性指标
all_predictions_for_density = []  # 存储所有电池的预测结果用于概率密度图
all_y_true_for_density = []  # 存储所有电池的真实值用于概率密度图
battery_names_list = []  # 存储电池名称
...
for battery_idx, (test_battery, (test_data, test_target)) in enumerate(battery_data.items()):
    ...
    # 绘制单个电池的概率密度图（学术论文风格）
    if battery_idx < 4:  # 只为前4个电池绘制单独的概率密度图
        plot_probability_density(predictions_array, test_np,
                                sample_idx=0, step_idx=0,
                                battery_name=f"Battery #{battery_idx + 5} (CNN-LSTM)")

    # 绘制训练损失图
    plt.figure(figsize=(12, 6))
    plt.plot(train_losses, label="Training Loss")
    plt.title(f"Training Loss over Epochs for {test_battery}", fontsize=16)
...
print(f"\nUncertainty Quantification Metrics (CNN-LSTM):")
